...
export class VariantService {
  ...
  async create(
    ...
    if (typeof attributes === 'string') {
      try {
        ...
        throw new BadRequestException('Format attributes is not valid');
      }
    }
  ...
  async findByProduct(productId: string) {
    ...
    if (!variants) {
      throw new NotFoundException('Không tìm thấy variant cho sản phẩm này');
    }
    return variants;
  }
  ...
  async update(
    ...
    if (!variant) {
      throw new NotFoundException('Variant is not exist');
    }
    const keptImages = dto.existingImages ? JSON.parse(dto.existingImages) : [];
    ...
    return { message: 'Update variant successfully' };
  }
